...
class TestNotifications:
    ...
    def test_list_own_notifications(self, authenticated_client, user, other_user):
        Notification.objects.create(user=user, type="system", title="Welcome!", body="Hello")
        Notification.objects.create(user=other_user, type="system", title="Their notif", body="Hi")
        response = authenticated_client.get(NOTIF_URL)
        assert response.data["count"] == 1

    def test_unread_count(self, authenticated_client, user):
        Notification.objects.create(user=user, type="system", title="N1", body="B1")
        Notification.objects.create(user=user, type="system", title="N2", body="B2", is_read=True)
        response = authenticated_client.get(UNREAD_URL)
        assert response.data["count"] == 1

    def test_mark_read(self, authenticated_client, user):
        notif = Notification.objects.create(user=user, type="system", title="N1", body="B1")
    ...
    def test_mark_all_read(self, authenticated_client, user):
        Notification.objects.create(user=user, type="system", title="N1", body="B1")
        Notification.objects.create(user=user, type="system", title="N2", body="B2")
